...
在此專案中，我分別將 ChatGPT 與 GitHub Copilot 應用於不同階段的開發任務，提升了架構設計品質與實作效率：

- **高層設計階段（需求拆解與功能架構）**  
  我以 ChatGPT 進行對話式構思，釐清核心流程如「訂單建立流程」、「後台管理界面需要的狀態切換」等，並請它協助生成初步的系統架構圖草稿、資料流思維圖，做為設計判斷依據。

- **技術決策與整合建議**  
  當我規劃如何串接 Google Sheets API 或設計後端資料寫入策略時，ChatGPT 提供了實作範例與安全性考量，如如何避免資料 race condition、如何以 POST 操作 gspread 寫入多欄資料。

- **底層實作與除錯過程**  
  在實作階段，我善用 GitHub Copilot 自動補全重複性邏輯（如表單處理、狀態切換 DOM 互動、Flask 路由處理），並觀察其建議碼是否符合專案需求。同時，我搭配 ChatGPT 分析錯誤訊息與 traceback，快速定位錯誤原因。

這樣的 AI 分工協作策略，幫助我專注在邏輯與整體設計思維，也強化了實作品質與開發節奏。
